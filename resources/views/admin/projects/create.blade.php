...
<div class="flex flex-1 flex-col gap-6">
    ...
    <form action="{{ route('admin.projects.store') }}" method="POST" class="mx-auto max-w-2xl space-y-6 rounded-xl border border-border bg-card p-6">
        @csrf
        <div>
            <label for="title" class="mb-2 block text-sm font-medium">Title *</label>
            <input id="title" name="title" type="text" required value="{{ old('title') }}" class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" placeholder="Project title" />
            @error('title')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
        </div>

        <div>
            <label for="description" class="mb-2 block text-sm font-medium">Description *</label>
            <textarea id="description" name="description" rows="4" required class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" placeholder="Project description">{{ old('description') }}</textarea>
        ...
        <div>
            ...
            <input type="hidden" name="tech_stack" id="tech-stack-input" value="{{ old('tech_stack') }}" />
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <input id="github_url" name="github_url" type="url" value="{{ old('github_url') }}" class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" placeholder="https://github.com/..." />
            ...
            <div>
                ...
                <input id="live_url" name="live_url" type="url" value="{{ old('live_url') }}" class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" placeholder="https://..." />
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <input id="display_order" name="display_order" type="number" value="{{ old('display_order', 0) }}" class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" />
            </div>
            <div class="flex items-end">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="featured" value="1" {{ old('featured') ? 'checked' : '' }} class="h-4 w-4 rounded border-border" />
        ...
        <div class="flex gap-3 pt-4">
            <button type="submit" class="rounded-md bg-primary px-6 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90">Create Project</button>
...
try {
    const oldVal = techStackInput.value;
    if (oldVal && oldVal !== '[]') {
        techStack = JSON.parse(oldVal);
        renderTags();
    }
} catch(e) {}
</script>
